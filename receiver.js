...
(function () {
  ...
  function isCustomFocusable(el) {
    if (!el) return false;
    if (focusableButtons.indexOf(el) !== -1) return true;
    return !!(el.closest && (el.closest('#mr-transport-controls') || el.closest('.mr-transport-controls')));
  }

  function ensureTransportFocus() {
    if (!isTransportVisible()) return;
    var activeEl = document.activeElement;
    if (!isCustomFocusable(activeEl)) {
  ...
  function normalizeRemoteKey(e) {
    ...
    if (key === 'Up' || key === 'UIUp' || key === 'NAVIGATE_UP' || key === 'DPAD_UP') return 'ArrowUp';
    if (key === 'Down' || key === 'UIDown' || key === 'NAVIGATE_DOWN' || key === 'DPAD_DOWN') return 'ArrowDown';
    if (key === 'Left' || key === 'UILeft' || key === 'NAVIGATE_LEFT' || key === 'DPAD_LEFT') return 'ArrowLeft';
    if (key === 'Right' || key === 'UIRight' || key === 'NAVIGATE_RIGHT' || key === 'DPAD_RIGHT') return 'ArrowRight';
    ...
    if (code === 413) return 'MediaStop';
    if (code === 179) return 'MediaPlayPause';
    if (code === 178) return 'MediaStop';
  ...
  function isOwnedRemoteKey(key) {
    return key === 'MediaPlayPause' || key === 'MediaPlay' || key === 'MediaPause' || key === 'MediaStop' ||
  ...
  window.addEventListener('keydown', function (e) {
    ...
    if (key === 'ArrowDown' || key === 'Escape' || key === 'Backspace') {
      if (!isTransportVisible() && key === 'ArrowDown') {
        showTransport();
        var btnsDown = getVisibleButtons();
        setFocus(Math.floor(btnsDown.length / 2));
      } else if (isTransportVisible()) {
        hideTransport();
      }
      return;
    }
  ...
  document.addEventListener('focusin', function (e) {
    if (!isTransportVisible()) return;
    if (isCustomFocusable(e.target)) return;
    ensureTransportFocus();
  }, true);
  ...
  setState('idle');
  log('lifecycle', 'Monitorr Cast Receiver v2.1.3 started (skipPlayersLoad=true, dummy element, no CAF overlay)');
