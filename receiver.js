...
//
// Uses the platform properly. No HLS.js, no skipPlayersLoad.
// Shaka (built-in) handles HLS playback.
// SEEK interceptor calls server-side seek + playerManager.load() for reload.
// Controls API for CC button, skip buttons, seek buttons.
// ─────────────────────────────────────────────────────────────────────────────
...
(function () {
  ...
  playerManager.setMessageInterceptor(
    ...
    function (request) {
      ...
      // Commands: always basic + dynamic skip based on episode context
      ...
      // Fetch duration + subtitle tracks async
      if (realDuration <= 0 && hlsSessionId && monitorrOrigin) fetchDuration();
      if (hlsSessionId && monitorrOrigin) fetchAndExposeSubtitleTracks();
  ...
  playerManager.setMessageInterceptor(
    ...
    function (request) {
      ...
      // DirectPlay: let default handler do it
      ...
      fetch(monitorrOrigin + '/api/cast/hls/' + hlsSessionId + '/seek?t=' + targetTime.toFixed(1), { method: 'POST' })
        ...
        .then(function (res) {
          console.log(TAG, 'Server seek OK, reloading');
          ...
          // Reload via playerManager.load() -- Shaka handles the new HLS source
          ...
          playerManager.load(loadReq).then(function () {
            serverSeeking = false;
            playerManager.broadcastStatus();
          }).catch(function (err) {
            console.error(TAG, 'Reload failed:', err);
            serverSeeking = false;
          });
        })
        .catch(function (err) {
  ...
  // ── EDIT_TRACKS (CC button) ────────────────────────────────────────────────

  playerManager.setMessageInterceptor(
    cast.framework.messages.MessageType.EDIT_TRACKS_INFO,
    function (request) {
      console.log(TAG, 'EDIT_TRACKS', JSON.stringify(request.activeTrackIds));
      ...
      if (activeIds.length === 0) {
        ...
        for (var i = 0; i < subtitleTracks.length; i++) {
          if (subtitleTracks[i].trackId === trackId) {
            enableSubs(subtitleTracks[i].streamIndex);
            break;
          }
        }
      }
    ...
    fetch(monitorrOrigin + '/api/cast/hls/' + hlsSessionId + '/subtitles')
      ...
      .then(function (data) {
        ...
        for (var i = 0; i < tracks.length; i++) {
          var t = tracks[i];
          var trackId = i + 1;
          subtitleTracks.push({ trackId: trackId, streamIndex: t.streamIndex, language: t.language });
